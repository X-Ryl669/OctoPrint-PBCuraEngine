$(function() {
    function PBCuraEngineViewModel(parameters) {
        var self = this;
        ...
        self.uploadDialog = $("#settings_plugin_PBCuraEngine_import");
        self.uploadElement = $("#settings-PBCuraEngine-import");
        ...
        self.removeProfile = function(data) {
            ...
            OctoPrint.slicing.deleteProfileForSlicer("PBCuraEngine", data.key, {url: data.resource()})
                .done(function() {
                    self.requestData();
                    self.slicingViewModel.requestData();
                });
        ...
        self.makeProfileDefault = function(data) {
            ...
            OctoPrint.slicing.updateProfileForSlicer("PBCuraEngine", data.key, {default: true}, {url: data.resource()})
                .done(function() {
                    self.requestData();
                });
        };
        ...
        self.testEnginePath = function() {
            OctoPrint.util.testExecutable(self.settings.plugins.PBCuraEngine.cura_engine())
        ...
        self.onAllBound = function() {
            self.uploadDialog.on("hidden", function(event) {
                if (event.target.id === "settings_plugin_PBCuraEngine_import") {
                    self.clearUpload();
                }
            });
        };
        ...
        self.onSlicingData = function(data) {
            if (data && data.hasOwnProperty("PBCuraEngine") && data.PBCuraEngine.hasOwnProperty("profiles")) {
                self.fromResponse(data.PBCuraEngine.profiles);
            }
        };
        ...
        self.onWizardDetails = function(response) {
            if (!response.hasOwnProperty("PBCuraEngine") || !response.PBCuraEngine.required) return;

            if (response.PBCuraEngine.details.hasOwnProperty("engine")) {
                self.unconfiguredCuraEngine(!response.PBCuraEngine.details.engine);
            }
            if (response.PBCuraEngine.details.hasOwnProperty("profile")) {
                self.unconfiguredSlicingProfile(!response.PBCuraEngine.details.profile);
            }
        };
    ...
    OCTOPRINT_VIEWMODELS.push({
        construct: PBCuraEngineViewModel,
        dependencies: ["loginStateViewModel", "settingsViewModel", "slicingViewModel"],
        elements: ["#settings_plugin_PBCuraEngine"]
    });
});
